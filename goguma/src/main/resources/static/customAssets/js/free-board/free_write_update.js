...
const mainCategoryId = document.querySelector("#main--cateogry--id");
const subCategoryId = document.querySelector("#sub--cateogry--id");
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    $.ajax({
		...
		success : function(data) {
			...
			// 초기값 지정
			let options = displayCategory(data , mainCategoryId.value);
			displayMainCategory.innerHTML = options;
			let sub = onclickMain(mainCategoryId.value);
...
// 카테고리 화면에 반영
const displayCategory = (data , value) => {
	let option = ""
	for(var i = 0; i < data.length; i++) {
		option += `<option value="${data[i].id}" ${value == data[i].id ? 'selected' : ''}>${data[i].name}</option>`;
	}
...
const onclickMain = (main) => {
	...
	$.ajax({
		...
		success : function(data) {
			...
			let options = displayCategory(data , subCategoryId.value);
